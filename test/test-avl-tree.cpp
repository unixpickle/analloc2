...
void TestBalancedInsertions() {
  ...
  assert(tree.GetAvlRoot()->right->GetValue() == 5);
  assert(tree.GetAvlRoot()->left->left->GetValue() == 0);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 4);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 6);
...
void TestBalancedDeletions() {
  ...
  assert(tree.GetAvlRoot()->left->right->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 4);
  assert(tree.GetAvlRoot()->right->right == nullptr);
  ...
  assert(tree.GetAvlRoot()->left->left->GetValue() == 0);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->left == nullptr);
  assert(tree.GetAvlRoot()->right->right == nullptr);
  assert(!tree.Remove(4));
  ...
  assert(tree.GetAvlRoot()->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->right->GetValue() == 5);
  assert(tree.GetAvlRoot()->left->left == nullptr);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->left == nullptr);
  assert(tree.GetAvlRoot()->right->right == nullptr);
  assert(!tree.Remove(0));
  ...
  assert(tree.GetAvlRoot()->right->GetValue() == 5);
  assert(tree.GetAvlRoot()->left->left == nullptr);
  assert(tree.GetAvlRoot()->left->right == nullptr);
  assert(tree.GetAvlRoot()->right->left == nullptr);
  assert(tree.GetAvlRoot()->right->right == nullptr);
  assert(!tree.Remove(2));
  ...
  assert(tree.GetAvlRoot()->GetValue() == 3);
  assert(tree.GetAvlRoot()->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->right == nullptr);
  assert(tree.GetAvlRoot()->left->left == nullptr);
  assert(tree.GetAvlRoot()->left->right == nullptr);
  assert(!tree.Remove(5));
  ...
  assert(aligner.GetAllocCount() == 1);
  assert(tree.GetAvlRoot()->GetValue() == 3);
  assert(tree.GetAvlRoot()->left == nullptr);
  assert(tree.GetAvlRoot()->right == nullptr);
  assert(!tree.Remove(1));
...
void TestBasicRootUnbalancedInsertions() {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    assert(tree.GetAvlRoot()->left->GetValue() == 1);
    assert(tree.GetAvlRoot()->right->GetValue() == 3);
    tree.Clear();
    assert(!tree.GetAvlRoot());
    assert(aligner.GetAllocCount() == 0);
  }
}
...
void TestBasicNonrootUnbalancedInsertions() {
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 0; j < 2; ++j) {
      ...
      assert(ValidateParent(tree.GetAvlRoot()->right));
      if (j == 0) {
        assert(tree.GetAvlRoot()->GetValue() == 4);
        assert(tree.GetAvlRoot()->right->GetValue() == 5);
        rotated = tree.GetAvlRoot()->left;
      } else {
        assert(tree.GetAvlRoot()->GetValue() == 0);
        assert(tree.GetAvlRoot()->left->GetValue() == -1);
        rotated = tree.GetAvlRoot()->right;
      }
...
void TestRandomInsertions() {
  ...
  for (int i = 0; i < 17; ++i) {
    ...
    if (i == 5) {
      ...
      assert(IsLeaf(tree.GetAvlRoot()->right->right));
      assert(IsLeaf(tree.GetAvlRoot()->left->left));
      assert(IsLeaf(tree.GetAvlRoot()->left->right));
      assert(ValidateParent(tree.GetAvlRoot()->left));
      assert(ValidateParent(tree.GetAvlRoot()->right));
      assert(ValidateParent(tree.GetAvlRoot()->left->left));
      assert(ValidateParent(tree.GetAvlRoot()->left->right));
      assert(ValidateParent(tree.GetAvlRoot()->right->right));
      assert(tree.GetAvlRoot()->GetValue() == 11);
      assert(tree.GetAvlRoot()->left->GetValue() == 6);
      assert(tree.GetAvlRoot()->right->GetValue() == 14);
      assert(tree.GetAvlRoot()->left->left->GetValue() == 5);
      assert(tree.GetAvlRoot()->left->right->GetValue() == 9);
      ...
      assert(IsLeftOnly(tree.GetAvlRoot()->left->left));
      assert(IsFull(tree.GetAvlRoot()->left->right));
      assert(IsLeaf(tree.GetAvlRoot()->right->left));
      assert(IsLeaf(tree.GetAvlRoot()->right->right));
      assert(IsLeaf(tree.GetAvlRoot()->left->left->left));
      assert(IsLeaf(tree.GetAvlRoot()->left->right->left));
      assert(IsLeaf(tree.GetAvlRoot()->left->right->right));
      assert(ValidateParent(tree.GetAvlRoot()->left));
      assert(ValidateParent(tree.GetAvlRoot()->right));
      assert(ValidateParent(tree.GetAvlRoot()->left->left));
      assert(ValidateParent(tree.GetAvlRoot()->left->right));
      assert(ValidateParent(tree.GetAvlRoot()->right->left));
      assert(ValidateParent(tree.GetAvlRoot()->right->right));
      assert(ValidateParent(tree.GetAvlRoot()->left->left->left));
      assert(ValidateParent(tree.GetAvlRoot()->left->right->left));
      assert(ValidateParent(tree.GetAvlRoot()->left->right->right));
      assert(tree.GetAvlRoot()->GetValue() == 11);
      assert(tree.GetAvlRoot()->left->GetValue() == 6);
      assert(tree.GetAvlRoot()->right->GetValue() == 15);
      assert(tree.GetAvlRoot()->left->left->GetValue() == 5);
      assert(tree.GetAvlRoot()->left->right->GetValue() == 9);
      assert(tree.GetAvlRoot()->right->left->GetValue() == 14);
      assert(tree.GetAvlRoot()->right->right->GetValue() == 17);
      assert(tree.GetAvlRoot()->left->left->left->GetValue() == 3);
      assert(tree.GetAvlRoot()->left->right->left->GetValue() == 7);
      ...
      assert(IsFull(tree.GetAvlRoot()->right));
      // Depth 2
      assert(IsFull(tree.GetAvlRoot()->left->left));
      assert(IsRightOnly(tree.GetAvlRoot()->left->right));
      assert(IsFull(tree.GetAvlRoot()->right->left));
      assert(IsLeftOnly(tree.GetAvlRoot()->right->right));
      // Depth 3
      assert(IsRightOnly(tree.GetAvlRoot()->left->left->left));
      assert(IsLeftOnly(tree.GetAvlRoot()->left->left->right));
      assert(IsLeaf(tree.GetAvlRoot()->left->right->right));
      assert(IsLeaf(tree.GetAvlRoot()->right->left->left));
      assert(IsFull(tree.GetAvlRoot()->right->left->right));
      assert(IsLeaf(tree.GetAvlRoot()->right->right->left));
      // Depth 4
      assert(IsLeaf(tree.GetAvlRoot()->left->left->left->right));
      assert(IsLeaf(tree.GetAvlRoot()->left->left->right->left));
      assert(IsLeaf(tree.GetAvlRoot()->right->left->right->left));
      assert(IsLeaf(tree.GetAvlRoot()->right->left->right->right));
      // Parental validation
      assert(ValidateParent(tree.GetAvlRoot()->left));
      assert(ValidateParent(tree.GetAvlRoot()->right));
      assert(ValidateParent(tree.GetAvlRoot()->left->left));
      ...
      assert(ValidateParent(tree.GetAvlRoot()->right->right));
      assert(ValidateParent(tree.GetAvlRoot()->left->left->left));
      assert(ValidateParent(tree.GetAvlRoot()->left->left->right));
      assert(ValidateParent(tree.GetAvlRoot()->left->right->right));
      assert(ValidateParent(tree.GetAvlRoot()->right->left->left));
      assert(ValidateParent(tree.GetAvlRoot()->right->left->right));
      assert(ValidateParent(tree.GetAvlRoot()->right->right->left));
      assert(ValidateParent(tree.GetAvlRoot()->left->left->left->right));
      assert(ValidateParent(tree.GetAvlRoot()->left->left->right->left));
      assert(ValidateParent(tree.GetAvlRoot()->right->left->right->left));
      assert(ValidateParent(tree.GetAvlRoot()->right->left->right->right));
      // Value validation
      assert(tree.GetAvlRoot()->GetValue() == 9);
      assert(tree.GetAvlRoot()->left->GetValue() == 6);
      assert(tree.GetAvlRoot()->right->GetValue() == 15);
      assert(tree.GetAvlRoot()->left->left->GetValue() == 3);
      assert(tree.GetAvlRoot()->left->right->GetValue() == 7);
      assert(tree.GetAvlRoot()->right->left->GetValue() == 11);
      assert(tree.GetAvlRoot()->right->right->GetValue() == 17);
      assert(tree.GetAvlRoot()->left->left->left->GetValue() == 1);
      assert(tree.GetAvlRoot()->left->left->right->GetValue() == 5);
      assert(tree.GetAvlRoot()->left->right->right->GetValue() == 8);
      assert(tree.GetAvlRoot()->right->left->left->GetValue() == 10);
      assert(tree.GetAvlRoot()->right->left->right->GetValue() == 13);
      assert(tree.GetAvlRoot()->right->right->left->GetValue() == 16);
      assert(tree.GetAvlRoot()->left->left->left->right->GetValue() == 2);
      assert(tree.GetAvlRoot()->left->left->right->left->GetValue() == 4);
      assert(tree.GetAvlRoot()->right->left->right->left->GetValue() == 12);
      assert(tree.GetAvlRoot()->right->left->right->right->GetValue() == 14);
    }
  }
}
...
void TestBalancedTrivialDeletions() {
  ...
  assert(tree.GetAvlRoot()->GetValue() == 2);
  assert(tree.GetAvlRoot()->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->right->GetValue() == 4);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
...
void TestUnbalancedTrivialDeletions() {
  ...
  assert(tree.Remove(1));
  assert(aligner.GetAllocCount() == 3);
  assert(ValidateRoot(tree.GetAvlRoot()));
  assert(IsFull(tree.GetAvlRoot()));
  assert(IsLeaf(tree.GetAvlRoot()->left));
  assert(IsLeaf(tree.GetAvlRoot()->right));
  assert(ValidateParent(tree.GetAvlRoot()->left));
  assert(ValidateParent(tree.GetAvlRoot()->right));
  assert(tree.GetAvlRoot()->GetValue() == 3);
  assert(tree.GetAvlRoot()->left->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->GetValue() == 4);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
  ...
  assert(IsFull(tree.GetAvlRoot()->left->left));
  assert(IsFull(tree.GetAvlRoot()->left->right));
  assert(IsLeftOnly(tree.GetAvlRoot()->right->left));
  assert(IsFull(tree.GetAvlRoot()->right->right));
  // Depth 3
  assert(IsLeftOnly(tree.GetAvlRoot()->left->left->left));
  assert(IsFull(tree.GetAvlRoot()->left->left->right));
  assert(IsLeftOnly(tree.GetAvlRoot()->left->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->right->right));
  assert(IsLeaf(tree.GetAvlRoot()->right->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->right->left));
  assert(IsRightOnly(tree.GetAvlRoot()->right->right->right));
  // Depth 4
  assert(IsLeaf(tree.GetAvlRoot()->left->left->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->left->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->left->right->right));
  assert(IsLeaf(tree.GetAvlRoot()->left->right->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->right->right->right));
  // Validate parents
  assert(ValidateParent(tree.GetAvlRoot()->left));
  assert(ValidateParent(tree.GetAvlRoot()->right));
  assert(ValidateParent(tree.GetAvlRoot()->left->left));
  ...
  assert(ValidateParent(tree.GetAvlRoot()->right->left->left));
  assert(ValidateParent(tree.GetAvlRoot()->right->right->left));
  assert(ValidateParent(tree.GetAvlRoot()->right->right->right));
  assert(ValidateParent(tree.GetAvlRoot()->left->left->left->left));
  assert(ValidateParent(tree.GetAvlRoot()->left->left->right->left));
  assert(ValidateParent(tree.GetAvlRoot()->left->left->right->right));
  assert(ValidateParent(tree.GetAvlRoot()->left->right->left->left));
  assert(ValidateParent(tree.GetAvlRoot()->right->right->right->right));
  // Validate values
  assert(tree.GetAvlRoot()->GetValue() == 13);
  assert(tree.GetAvlRoot()->left->GetValue() == 8);
  assert(tree.GetAvlRoot()->right->GetValue() == 16);
  assert(tree.GetAvlRoot()->left->left->GetValue() == 4);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 11);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 15);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 18);
  assert(tree.GetAvlRoot()->left->left->left->GetValue() == 3);
  assert(tree.GetAvlRoot()->left->left->right->GetValue() == 6);
  assert(tree.GetAvlRoot()->left->right->left->GetValue() == 10);
  assert(tree.GetAvlRoot()->left->right->right->GetValue() == 12);
  assert(tree.GetAvlRoot()->right->left->left->GetValue() == 14);
  assert(tree.GetAvlRoot()->right->right->left->GetValue() == 17);
  assert(tree.GetAvlRoot()->right->right->right->GetValue() == 19);
  assert(tree.GetAvlRoot()->left->left->left->left->GetValue() == 2);
  assert(tree.GetAvlRoot()->left->left->right->left->GetValue() == 5);
  assert(tree.GetAvlRoot()->left->left->right->right->GetValue() == 7);
  assert(tree.GetAvlRoot()->left->right->left->left->GetValue() == 9);
  assert(tree.GetAvlRoot()->right->right->right->right->GetValue() == 20);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
  ...
  // Depth 3
  assert(IsLeaf(tree.GetAvlRoot()->left->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->left->right));
  assert(IsFull(tree.GetAvlRoot()->left->right->left));
  assert(IsFull(tree.GetAvlRoot()->left->right->right));
  assert(IsLeaf(tree.GetAvlRoot()->right->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->left->right));
  assert(IsLeaf(tree.GetAvlRoot()->right->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->right->right));
  // Depth 4
  assert(IsLeaf(tree.GetAvlRoot()->left->right->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->right->left->right));
  assert(IsLeaf(tree.GetAvlRoot()->left->right->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->right->right->right));
  // Validate parents
  assert(ValidateParent(tree.GetAvlRoot()->left));
  assert(ValidateParent(tree.GetAvlRoot()->right));
  assert(ValidateParent(tree.GetAvlRoot()->left->left));
  ...
  assert(ValidateParent(tree.GetAvlRoot()->right->right->right));
  assert(ValidateParent(tree.GetAvlRoot()->left->right->left->left));
  assert(ValidateParent(tree.GetAvlRoot()->left->right->left->right));
  assert(ValidateParent(tree.GetAvlRoot()->left->right->right->left));
  assert(ValidateParent(tree.GetAvlRoot()->left->right->right->right));
  // Validate values
  assert(tree.GetAvlRoot()->GetValue() == 13);
  assert(tree.GetAvlRoot()->left->GetValue() == 5);
  assert(tree.GetAvlRoot()->right->GetValue() == 17);
  assert(tree.GetAvlRoot()->left->left->GetValue() == 2);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 9);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 15);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 19);
  assert(tree.GetAvlRoot()->left->left->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->left->left->right->GetValue() == 3);
  assert(tree.GetAvlRoot()->left->right->left->GetValue() == 7);
  assert(tree.GetAvlRoot()->left->right->right->GetValue() == 11);
  assert(tree.GetAvlRoot()->right->left->left->GetValue() == 14);
  assert(tree.GetAvlRoot()->right->left->right->GetValue() == 16);
  assert(tree.GetAvlRoot()->right->right->left->GetValue() == 18);
  assert(tree.GetAvlRoot()->right->right->right->GetValue() == 20);
  assert(tree.GetAvlRoot()->left->right->left->left->GetValue() == 6);
  assert(tree.GetAvlRoot()->left->right->left->right->GetValue() == 8);
  assert(tree.GetAvlRoot()->left->right->right->left->GetValue() == 10);
  assert(tree.GetAvlRoot()->left->right->right->right->GetValue() == 12);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
  ...
  assert(IsLeaf(tree.GetAvlRoot()->right->right->left));
  assert(IsFull(tree.GetAvlRoot()->right->left->right));
  assert(IsFull(tree.GetAvlRoot()->right->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->right->right));
  assert(IsLeaf(tree.GetAvlRoot()->left->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->left->left->right));
  assert(IsLeaf(tree.GetAvlRoot()->left->left->left));
  ...
  assert(IsLeaf(tree.GetAvlRoot()->right->left->right->right));
  assert(IsLeaf(tree.GetAvlRoot()->right->left->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->left->left->right));
  assert(IsLeaf(tree.GetAvlRoot()->right->left->left->left));
  // Validate parents
  assert(ValidateParent(tree.GetAvlRoot()->right));
  assert(ValidateParent(tree.GetAvlRoot()->left));
  assert(ValidateParent(tree.GetAvlRoot()->right->right));
  ...
  assert(ValidateParent(tree.GetAvlRoot()->right->left->right->right));
  assert(ValidateParent(tree.GetAvlRoot()->right->left->right->left));
  assert(ValidateParent(tree.GetAvlRoot()->right->left->left->right));
  assert(ValidateParent(tree.GetAvlRoot()->right->left->left->left));
  // Validate values
  assert(tree.GetAvlRoot()->GetValue() == 8);
  assert(tree.GetAvlRoot()->left->GetValue() == 4);
  assert(tree.GetAvlRoot()->right->GetValue() == 16);
  // Depth 2
  assert(tree.GetAvlRoot()->left->left->GetValue() == 2);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 6);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 12);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 19);
  // Depth 3
  assert(tree.GetAvlRoot()->left->left->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->left->left->right->GetValue() == 3);
  assert(tree.GetAvlRoot()->left->right->left->GetValue() == 5);
  assert(tree.GetAvlRoot()->left->right->right->GetValue() == 7);
  assert(tree.GetAvlRoot()->right->left->left->GetValue() == 10);
  assert(tree.GetAvlRoot()->right->left->right->GetValue() == 14);
  assert(tree.GetAvlRoot()->right->right->left->GetValue() == 18);
  assert(tree.GetAvlRoot()->right->right->right->GetValue() == 20);
  // Depth 4
  assert(tree.GetAvlRoot()->right->left->left->left->GetValue() == 9);
  assert(tree.GetAvlRoot()->right->left->left->right->GetValue() == 11);
  assert(tree.GetAvlRoot()->right->left->right->left->GetValue() == 13);
  assert(tree.GetAvlRoot()->right->left->right->right->GetValue() == 15);
}
...
void TestBalancedNontrivialDeletions() {
  ...
  assert(ValidateRoot(tree.GetAvlRoot()));
  assert(IsFull(tree.GetAvlRoot()));
  assert(IsLeftOnly(tree.GetAvlRoot()->left));
  assert(IsFull(tree.GetAvlRoot()->right));
  assert(IsLeaf(tree.GetAvlRoot()->left->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->right));
  ...
  assert(tree.GetAvlRoot()->GetValue() == 3);
  assert(tree.GetAvlRoot()->left->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->GetValue() == 6);
  assert(tree.GetAvlRoot()->left->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 5);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 7);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
  ...
  assert(ValidateRoot(tree.GetAvlRoot()));
  assert(IsFull(tree.GetAvlRoot()));
  assert(IsRightOnly(tree.GetAvlRoot()->left));
  assert(IsFull(tree.GetAvlRoot()->right));
  assert(IsLeaf(tree.GetAvlRoot()->left->right));
  assert(IsLeaf(tree.GetAvlRoot()->right->left));
  assert(IsLeaf(tree.GetAvlRoot()->right->right));
  ...
  assert(tree.GetAvlRoot()->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->right->GetValue() == 6);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 3);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 5);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 7);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
  ...
  assert(tree.GetAvlRoot()->GetValue() == 4);
  assert(tree.GetAvlRoot()->left->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->GetValue() == 7);
  assert(tree.GetAvlRoot()->left->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 3);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 6);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 8);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
  ...
  assert(ValidateParent(tree.GetAvlRoot()->right->right));
  // Validate values
  assert(tree.GetAvlRoot()->GetValue() == 5);
  assert(tree.GetAvlRoot()->left->GetValue() == 2);
  assert(tree.GetAvlRoot()->right->GetValue() == 7);
  assert(tree.GetAvlRoot()->left->left->GetValue() == 1);
  assert(tree.GetAvlRoot()->left->right->GetValue() == 4);
  assert(tree.GetAvlRoot()->right->left->GetValue() == 6);
  assert(tree.GetAvlRoot()->right->right->GetValue() == 8);
  tree.Clear();
  assert(aligner.GetAllocCount() == 0);
...
void TestRandomModifications() {
  ...
  for (size_t i = 0; i < sizeof(modifications) / sizeof(int); ++i) {
    ...
    int ignored;
    assert(ValidateBalance(tree.GetAvlRoot(), ignored));
  }
