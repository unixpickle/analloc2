...
void TestRandomAlign() {
  ...
  assert(aligner.OffsetAlign(buffer, regionSize * 0x100, preambleSize,
         totalSize));
  
  Pfl * pfl = Pfl::Place(buffer, preambleSize + regionSize * 0x100);
  assert(pfl != nullptr);
  assert(pfl->GetStackCount() == 1);
  
  uintptr_t start = buffer + preambleSize;
